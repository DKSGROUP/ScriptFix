...
if(count($faceImages)>0)
{
    ...
    //move to the folder
    foreach ($faceImages as $key => $value) {
        $d = postYandex('https://cloud-api.yandex.net/v1/disk/resources/move?from=/Downloads/photty_nova_'.$name.$face.$key.'.jpg'.'&path=/phottyru/findface/'.$name.'/face'.$face.'/'.$value['name'],array(),$ydisk['access_token']);
    }
    ...
    $down = file_get_contents('https://cloud-api.yandex.net/v1/disk/public/resources/download?public_key='.urlencode($raw_data));
    $down = json_decode($down,true);
    echo $down['href'];
}
?>